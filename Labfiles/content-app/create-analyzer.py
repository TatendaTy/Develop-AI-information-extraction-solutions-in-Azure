...
def create_analyzer (schema, analyzer, endpoint, key):
    ...
    # now create the analyzer
    response = requests.put(url, headers=headers, data=(schema)) # initiate the creation of the analyzer.
    print(response.status_code)

    # get the response and extract the callback URL
    callback_url = response.headers["Operation-Location"] # Checks the response to retrieve the Operation-Location callback URL
    ...
    if result == "Succeeded":
        ...
        print(f"Analyzer {analyzer} creation failed.")
